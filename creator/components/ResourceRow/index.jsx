...
export function ResourceRow({ onDelete, index, rowData }) {
  ...
  useEffect(() => {
    ...
    console.log({
      target: targetImageRef.current,
      source: sourceImageRef.current,
    });
  }, [sourceImage, targetImageData]);

  const hiddenFilePicker = useRef(null);
  const targetImageRef = useRef(null),
    sourceImageRef = useRef(null);
    ...
    >
      ...
      <Stack flexGrow={1}>
        ...
        <Image
          ...
          src={
            sourceImage
              ? `https://autosam.github.io/Tamaweb/${sourceImage}`
      ...
      <Stack flexGrow={1}>
        <img
          ...
          src={targetImageData || "static/images/default.jpg"}
        ></img>
        ...
        <Typography
          fontSize={10}
          px={1}
          bgcolor={isMB ? "darkred" : "white"}
          position={"absolute"}
        >
          {fileSize}
          {Boolean(targetImageRef.current) &&
            ` (${targetImageRef.current?.naturalWidth}x${targetImageRef.current?.naturalHeight})`}
        </Typography>
      </Stack>
